...
这些文件属于 A 的公共示例目录，不是四个业务模块的真实实现。所有类都显式 `is_mock = True`；即使通过路径配置加载，API 仍显示 Mock。

```powershell
uv sync --locked
uv run python -m scripts.check_member resume --examples
uv run python -m scripts.check_member jobs --examples
uv run python -m scripts.check_member diagnosis --examples
uv run python -m scripts.check_member analytics --examples
```
...
- 空技能 JD 的分数策略由 D 说明，必须避免除零，不编造缺失技能。
- analytics 的这组样例按“包含该技能的岗位数”统计：Python=2、SQL=2、Docker=1；空岗位返回空字典。复杂同义词、薪资和图表口径另行约定。
- diagnosis 的文字是固定离线响应，只验证响应结构。真实建议的质量需要人工验收，不能靠等于样例文字判断。
