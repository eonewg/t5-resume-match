...
def probe(role: str, target: str, *, examples=False, live=False):
    ...
    if role == "diagnosis" and not (examples or live):
...
def module_tests_exist(role, root=ROOT):
    directory = root / MODULES[role].tests
    require(
        directory.is_dir() and any(directory.rglob("test_*.py")),
        f"缺少 {MODULES[role].tests}/test_*.py；不能只靠公共测试交付",
    )
...
def run_probe(role, args):
    target = args.provider or (MODULES[role].example if args.examples else MODULES[role].entrypoint)
...
def run_module_tests(role, *, timeout):
    ...
    with tempfile.TemporaryDirectory(prefix="t5-member-") as temporary:
        ...
        run(
            [sys.executable, "-m", "pytest", MODULES[role].tests, "-q", f"--junitxml={report}"],
            timeout=timeout,
        )
...
def ci_roles(ref: str, root=ROOT):
    if ref == BRANCHES["D"]:
        ...
        return [key for key, spec in MODULES.items() if spec.owner == "D"]
    require(ref in ("main", BRANCHES["A"]), f"未知成员分支 {ref}，请使用团队约定的分支名")
    return [key for key, spec in MODULES.items() if (root / f"backend/modules/{key}").exists()]


def main(argv=None):
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("role", nargs="?", choices=MODULES)
    parser.add_argument(
    ...
    if args.ci:
        ...
        for role in roles:
            ...
            run_probe(role, args)
        return 0
    require(args.role is not None, "请指定 resume/jobs/diagnosis/analytics 或 --ci")
    if args._probe:
