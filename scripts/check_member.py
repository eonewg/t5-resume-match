"""One-command member smoke gate. It is not an A acceptance verdict.

Examples: uv run python -m scripts.check_member B --examples
Real B:   uv run python -m scripts.check_member B
D is offline by default; --live explicitly permits a real diagnosis request.
"""
...
def probe(role: str, target: str, *, examples=False, live=False):
    ...
    if role == "D" and not (examples or live):
        return "OFFLINE: D 仅检查入口与同步签名；不构造实例、不调用 AI。需模块离线测试验证输出。"
    ...
    if role == "B":
        ...
        require(result.raw_text == resume.raw_text, "B 必须保留输入原文")
        require("python" in skill_set(result.skills), "B 未提取样例明确列出的 Python 技能")
        short = "未提供学历和技术经历。"
        result = validate(ResumeData, service.parse(TextInput(raw_text=short)))
        require(result.raw_text == short, "B 对缺失字段输入也必须保留原文")
    elif role == "C":
        for jd in jobs:
            ...
            require(skill_set(result.skills) == skill_set(jd.skills), "C 的样例 JD 技能提取不一致")
        for case in samples["match_cases"]:
            ...
            require(
                result.resume_id == left.id and result.jd_id == right.id, "C 返回了错误的关联 ID"
            )
            require(
                skill_set(result.matched_skills) == skill_set(case["matched_skills"]),
                f"C {case['name']} 的 matched_skills 不一致",
            )
            require(
                skill_set(result.missing_skills) == skill_set(case["missing_skills"]),
                f"C {case['name']} 的 missing_skills 不一致",
            )
            require(
                before == (left.model_dump(), right.model_dump()), "C 不得原地修改传入的简历/JD"
            )
    elif role == "D":
        ...
        require(counts == expected, "E 样例技能统计应为 Python=2、SQL=2、Docker=1（岗位数）")
        empty = validate(AnalysisResult, service.analyze([]))
        require(empty.skills == {}, "E 无岗位时应返回空统计")
    return (
        "EXAMPLE_CHECK_PASS（固定示例，不是业务验收）" if examples else "PUBLIC_CONTRACT_CHECK_PASS"
    )


def module_tests_exist(role, root=ROOT):
    directory = root / MEMBERS[role].tests
    require(
        directory.is_dir() and any(directory.rglob("test_*.py")),
        f"缺少 {MEMBERS[role].tests}/test_*.py；不能只靠公共测试交付",
    )
...
def run_probe(role, args):
    target = args.provider or (MEMBERS[role].example if args.examples else MEMBERS[role].entrypoint)
    command = [sys.executable, "-m", "scripts.check_member", role, "--_probe", "--provider", target]
    if args.examples:
...
def run_module_tests(role, *, timeout):
    module_tests_exist(role)
    # A separate module report prevents an empty/skipped module passing on core tests alone.
    with tempfile.TemporaryDirectory(prefix="t5-member-") as temporary:
        report = Path(temporary) / "tests.xml"
        run(
            [sys.executable, "-m", "pytest", MEMBERS[role].tests, "-q", f"--junitxml={report}"],
            timeout=timeout,
        )
        ...
        require(
            ...
            "成员测试没有实际通过的用例（全部跳过或未收集）",
        )
    run([sys.executable, "-m", "pytest", "tests/core", "-q"], timeout=timeout)


def ci_roles(ref: str, root=ROOT):
    for role, spec in MEMBERS.items():
        if ref == spec.branch:
            return [role]
    require(ref in ("main", "feat/core-a"), f"未知成员分支 {ref}，请使用团队约定的分支名")
    return [
        role for role, spec in MEMBERS.items() if (root / f"backend/modules/{spec.key}").exists()
    ]
...
def main(argv=None):
    ...
    parser.add_argument(
        "--examples", action="store_true", help="只检查固定接入示例，不代表成员实现"
    )
    parser.add_argument("--provider", help="替换公开入口 module:Class（不修改 .env）")
    parser.add_argument("--live", action="store_true", help="明确允许 D 调用真实 AI，可能产生费用")
    ...
    if args.ci:
        require(
            not args.examples and not args.live and not args.provider and not args.role,
            "--ci 不与角色/示例/真实 AI 参数混用",
        )
        ref = os.environ.get("GITHUB_HEAD_REF") or os.environ.get("GITHUB_REF_NAME", "")
        roles = ci_roles(ref)
        if not roles:
            print("CI: 尚无成员模块，公共骨架检查不代表成员交付。")
        for role in roles:
            module_tests_exist(role)
            run_probe(role, args)
        return 0
    require(args.role is not None, "请指定 B/C/D/E 或 --ci")
    if args._probe:
        print(probe(args.role, args.provider, examples=args.examples, live=args.live))
        return 0
    if not args.examples:
        module_tests_exist(args.role)
    run_probe(args.role, args)
    if not args.examples:
        run_module_tests(args.role, timeout=max(args.timeout, 180))
        print("MEMBER_CHECK_PASS：公开自检和测试通过；A 仍需审查实现、范围及业务效果。")
    return 0
