...
try {
    ...
    uv run --locked --group build pyinstaller --noconfirm --clean --onedir --console `
        --name Vitae --distpath $DistPath --workpath $WorkPath --specpath $WorkPath --paths . `
    ...
    Set-Content -LiteralPath (Join-Path $DistPath 'Vitae/.env.example') -Encoding ascii `
        -Value 'DEEPSEEK_API_KEY='
    Copy-Item -LiteralPath 'docs/windows-quickstart.txt' `
        -Destination (Join-Path $DistPath 'Vitae/使用说明.txt')
    Write-Host (Join-Path $DistPath 'Vitae/Vitae.exe')
} finally {
    Pop-Location
}
