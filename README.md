...
PR #8 已合入 feat/core-a，merge 为 `a41a31d22dac3b31cb7cac8303f63cdd799ecab4`。安装、完整回归与真实调用结果见 [最终安装记录](docs/final/fresh-install.md)，五页演示见 [演示脚本](docs/final/demo-script.md)。另见 [补充 STAR 验证](docs/final/star-supplement.md)与[公开竞品调研](docs/competitor-analysis.md)：补充两次请求失败、无可评改写，竞品不冒充人工实测。最终以草稿 PR 提交审查，不自动合 main。见 [交付状态与已知限制](docs/final/delivery-status.md)、[独立 AI 盲评](docs/final/ai-evaluation.md)、[新增真实 JD / 薪资说明](data/market/2026-09-08/README.md)。新增五份摘要通过 `uv run --locked python -m scripts.import_final_samples --apply` 导入，和默认五份 Canonical 快照合计十条、六家雇主。
