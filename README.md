...
模块详细边界、接入示例和数据库说明见 [架构](docs/architecture.md)。业务成员的请求放入 `docs/integration_requests/`，由 A 实施公共变更。不要把根 AGENTS.md 的 A 固定分支规则当成 B/C/D/E 的开发规则；其他角色开始前应由团队明确其适用角色指令。
