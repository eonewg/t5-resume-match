...
按 [T5 需求对照表](T5_REQUIREMENTS_MATRIX.md) 和 [两人分工](T5_TWO_PERSON_ALLOCATION_STRICT.md) 开发：A 负责公共平台、resume、analytics、数据库与最终交付；D 负责 jobs/matching、embedding、diagnosis。当前默认接入真实规则 Resume 解析与 Jobs 关键词匹配，Jobs 已正式挂载；PostgreSQL + pgvector 公共设施已真实验证。Resume 专用编辑器 UI、市场分析、Embedding 与真实 AI 验收仍待后续完成，完整状态见 [验收台账](docs/acceptance.md)。
...
需要修改配置时复制 `.env.example` 为 `.env`；不复制也能启动。环境变量优先于 `.env`。Resume/Jobs 默认使用已集成的真实规则 provider，Diagnosis/Analytics 仍为 Mock。旧 `.env` 若显式设置了 `T5_JOBS_PROVIDER=mock`，需改为下表正式入口；不会静默覆盖用户配置。类实现 [ports.py](backend/core/ports.py) 中同步接口，构造函数无参数。
...
PostgreSQL/pgvector 依赖已进入默认锁定依赖。启动、迁移、原生 Windows / Compose 配置、向量写入与相似度查询见 [数据库与向量契约](docs/postgres.md)。已真实验证 PostgreSQL 17.6 + pgvector 0.8.1，但尚未接入 D 的实际 embedding 模型。SQLite 演示不会自动转换成 PostgreSQL 数据。
