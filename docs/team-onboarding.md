...
先按 [成员开发与交付](member-development.md) 跑通自己的接入示例，再实现模块；提交前运行一条 `check_member` 自检命令。样例通过不代表业务已验收。
...
4. UI 开发使用已有 [公共前端壳](frontend-integration.md)，在自己的模块目录导出 mount 入口，通过专用 URL 预览，不需要改 A 的注册表或另建工程。
