...
| `POST /api/v1/jobs/preview` | `{"raw_text":"岗位原文"}` | 200，JDCreate 草稿，保留 original_text；不写库 |
| `POST /api/v1/jobs/extract-text` | multipart/form-data，`file`，PDF/DOCX/TXT | 200，`{"raw_text":"提取文字"}`；不调用 AI、不写库 |
