...
const sourceLabels = {real: '真实采样', course: '课程样本', synthetic: '合成演示', unknown: '来源未确认'};
...
export function mount(container, context) {
  ...
  const header = node('div'); header.append(node('p', '从已录入岗位出发', 'eyebrow'), node('h1', '市场分析'),
  ...
  for (const [value, label] of [['real', '真实采样'], ['', '全部来源'], ['course', '课程样本'], ['synthetic', '合成演示'], ['unknown', '来源未确认']]) {
  ...
  library.append(node('summary', '使用已归档的真实 JD'));
  library.append(node('p', '导入 2026 年 9 月 8 日采集的 5 份 Canonical 招聘快照。均来自同一雇主，薪资未披露；不能代表整个就业市场，也不代表岗位现在仍开放。'));
  const importButton = node('button', '导入 5 份真实 JD 快照', 'button secondary'); importButton.type = 'button'; importButton.id = 'analytics-import'; library.append(importButton);
  ...
  function renderResult(result) {
    ...
    if (!result) return;
    if (result.is_mock) body.append(node('p', '当前分析包含 Mock 模块或输入，请勿用于真实市场结论。', 'notice'));
    const data = result.market;
    if (!data) { body.append(node('p', '当前分析模块未提供完整市场指标，请配置真实 Analytics 模块。', 'notice')); return; }
    ...
    for (const [value, title, detail] of [
      [data.sample_size, 'JD 样本', `当前筛选 / 已录入 ${result.scope?.available_count ?? data.sample_size} 条`],
      [data.skill_frequency.length, '技能 / 工具', '按 JD 去重计数，不重复叠加 tools'],
    ...
    if (result.scope?.excluded_mock_count) body.append(node('p', `已排除 ${result.scope.excluded_mock_count} 条标记为 Mock 的记录。`, 'analytics-help'));
    if (!data.sample_size) {
      const empty = card('还没有可分析的岗位'); empty.id = 'analytics-empty';
      empty.append(node('p', result.summary), node('p', '可以展开上方快照入口导入真实 JD，或切换到“全部来源”查看手动录入的岗位。'));
      body.append(empty); return;
    }
    const chartGrid = node('div', '', 'analytics-chart-grid');
    const cloud = card('技能词云', `字号随出现该技能的 JD 数量变化。展示前 ${Math.min(30, data.skill_frequency.length)} 项。`);
    ...
    for (const row of data.skill_frequency.slice(0, 30)) {
      ...
      item.title = `${row.skill}：${row.job_count}/${data.sample_size} 条 JD，${row.share_percent}%`; cloudList.append(item);
    }
    cloud.append(cloudList);
    if (!data.skill_frequency.length) cloud.append(node('p', '尚无已识别或确认的技能关键词。'));
    const frequency = card('技能要求分布', `分母为当前 ${data.sample_size} 条 JD。同一岗位可要求多种技能；展示前 ${Math.min(15, data.skill_frequency.length)} 项。`);
    ...
    allSkills.append(table(['技能 / 工具', 'JD 数量', '占样本比例'], data.skill_frequency.map(row => [row.skill, String(row.job_count), `${row.share_percent}%`])));
    frequency.append(allSkills); chartGrid.append(cloud, frequency); body.append(chartGrid);
    ...
    if (!data.salary_groups.length) salary.append(node('p', '当前样本没有可比较的薪资区间。保留 unknown，不生成推测分布。', 'analytics-empty-salary'));
    ...
    body.append(salary);
    const sourceCard = card('岗位与来源明细', '统计单位是已保存 JD 记录；重复手动录入会分别计数。固定快照导入会去重。来源标签来自录入资料，不等于独立事实核验。');
    ...
    sourceCard.append(table(['岗位 / 雇主', '已确认或解析的技能', '来源与采集日期', '薪资状态'], data.jobs.map(job => {
      ...
      const salaryText = {comparable: '已进入对应分组', missing_range: 'unknown · 无完整区间', missing_unit: 'unknown · 单位未确认'}[job.salary_status];
  ...
  const controller = connectAnalytics(context, state => {
    ...
    status.textContent = state.error || state.notice || (state.busy === 'import' ? '正在导入真实快照…' : state.busy ? '正在计算当前筛选…' : '已更新。');
