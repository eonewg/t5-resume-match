...
const report={widths:viewports.widths,checks:[],status:'running'};
...
(async()=>{
 ...
 try{
  for(const width of report.widths){
   ...
   assert.equal(await page.locator('#resume-name').inputValue(),'');await shot(page,'pdf-no-text-'+width);
   for(const file of [
   ...
   ]){await page.locator('#resume-file').setInputFiles(file);await page.locator('#resume-status[data-state=error]').waitFor();assert.equal(await page.locator('#resume-name').inputValue(),'');}
   ...
   const paste=await browser.newPage({viewport:{width,height:viewports.height(width)}});await paste.goto(base+'/#resume');await paste.locator('#resume-dropzone:enabled').waitFor();
   if (!(await paste.locator('#resume-raw').isVisible())) await paste.locator('.resume-source summary').first().click();await paste.locator('#resume-raw').fill(raw);await paste.locator('#resume-parse').click();
