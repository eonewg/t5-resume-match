...
(async()=>{
 ...
 const base=process.env.T5_SMOKE_URL||'http://127.0.0.1:8770';const out='.verification/ui-refresh';fs.mkdirSync(out,{recursive:true});
 ...
 try{
  ...
  await page.locator('#diagnosis-run').click();assert.equal(await page.locator('#diagnosis-run').isDisabled(),true);await page.getByTestId('diagnosis-result-mode').filter({hasText:'演示数据'}).waitFor();
