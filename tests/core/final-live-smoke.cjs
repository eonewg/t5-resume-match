...
(async () => {
  ...
  try {
    ...
    await page.getByRole('button',{name:'解析并保存 JD',exact:true}).click();await page.locator('#jobs-status').filter({hasText:'JD 已解析并保存'}).waitFor();
    await page.getByRole('button',{name:'计算匹配',exact:true}).click();await page.locator('#jobs-result:not([hidden])').waitFor();
    ...
    const started=Date.now();await page.getByRole('button',{name:'诊断当前简历',exact:true}).click();
    ...
    await page.getByTestId('diagnosis-result-mode').filter({hasText:'AI 诊断结果'}).waitFor();
    ...
    await page.getByRole('button',{name:'诊断当前简历',exact:true}).click();await page.getByRole('status').filter({hasText:'受控失败'}).waitFor();
    assert.equal(await page.getByTestId('diagnosis-result').isVisible(),false);await page.unroute('**/api/v1/diagnoses');
    await page.getByRole('button',{name:'诊断当前简历',exact:true}).click();await page.getByTestId('diagnosis-result-mode').filter({hasText:'AI 诊断结果'}).waitFor();
